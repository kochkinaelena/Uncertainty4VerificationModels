...
#    for k in list(tree_results.keys()):
    for k,v in sorted(tree_results.items()):
...
for fold in folds:
    ...
    X_train,Y_train, ids_train = pred2feats(predictions, 'train')
    print ("Train set class balance", np.unique(Y_train, return_counts='True'))
    
    X_dev,Y_dev,ids_dev = pred2feats(predictions, 'dev')
    print ("Dev set class balance", np.unique(Y_dev, return_counts='True'))
    
    with open("xdev1.txt","wb") as fout:
        pickle.dump(X_dev,fout )
    with open("ydev1.txt","wb") as fout:
        pickle.dump(Y_dev, fout)
    with open("idsdev1.txt","wb") as fout:
        pickle.dump(ids_dev, fout)
    
    X_test,Y_test, ids_test = pred2feats(predictions, 'test')
    print ("Test set class balance", np.unique(Y_test, return_counts='True'))
#    random_state = np.random.RandomState(seed=0)
    clf = SVC(kernel='linear', gamma='auto',random_state=random_state, class_weight='balanced', probability=True) # kernel='linear', default is 'rbf'
    clf.fit(X_dev, Y_dev) 
...
#    random_state = np.random.RandomState(seed=0)
    clf = RandomForestClassifier(n_estimators=100, max_depth=2,random_state=random_state,class_weight='balanced',n_jobs=1)
    clf.fit(X_dev, Y_dev) 
